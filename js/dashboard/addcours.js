  ...
  holder: editorTemplate, 
   tools: {
    ...
    image: {
      ...
      config: {
        endpoints: {
          byFile: 'http://localhost:8000/dashboard/uploadFile', // Your backend file uploader endpoint
...
document.getElementById("submit-course-btn").addEventListener("click", async () => {
  ...
  const result = await fetch("http://localhost:8000/dashboard/teacher/addcours", {
    method: "POST",
    headers: {
      "content-type": "application/json",
  ...
  if (result.status == 200) {
    ...
    const resultat = await fetch(`http://localhost:8000/dashboard/image/cours/${response.id}`, {
        "method":"POST",
        "body": cleanForm,
    });
